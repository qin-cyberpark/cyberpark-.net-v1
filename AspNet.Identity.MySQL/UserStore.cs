...
namespace AspNet.Identity.MySQL
{
        ...
        where TUser : IdentityUser
    {
        ...
        /// <param name="email"></param>
        /// <returns></returns>
        public Task<TUser> FindByNameOrEmailAsync(string nameOrEmail)
        {
            if (String.IsNullOrEmpty(nameOrEmail))
            {
                throw new ArgumentNullException("name or email");
            }

            List<TUser> result = userTable.GetUserByNameOrEmail(nameOrEmail) as List<TUser>;
        ...
        public Task<TUser> FindByPhoneOrEmailAsync(string phoneOrEmail)
        {
            if (String.IsNullOrEmpty(phoneOrEmail))
            {
                throw new ArgumentNullException("phone or email");
            }

            List<TUser> result = userTable.GetUserByPhoneOrEmail(phoneOrEmail) as List<TUser>;
            if (result != null && result.Count > 0)
            {
                return Task.FromResult<TUser>(result[0]);
            }
